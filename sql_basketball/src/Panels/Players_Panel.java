...
public class Players_Panel extends JPanel implements Paths_NBA {
	...
	public Players_Panel(ArrayList<String> result)
		...
		panel.setPreferredSize(new Dimension(500, 55* playersNumber));

		final JScrollPane scroll = new JScrollPane(panel);
		JLabel temp;
		GridLayout gl = new GridLayout(playersNumber, 1 , 2, 2);
		panel.setLayout(gl);
		...
		while (i < result.size()) {
			...
			try {
				...
				playerPicture = ImagesGui.resize(playerPicture, 30, 30);
				...
				if (FilesUtils.isDirectoryExists(PLAYERS_IMAGES_CACHE)
					...
					url = new URL("https://www.esportsearnings.com/images/unknown_player.png");
					playerPicture = ImageIO.read(url);
			...
			temp = new JLabel(tempPlayer.firstName + " " + tempPlayer.lastName, imageIcon, SwingConstants.LEFT);
